...
impl Client {
    pub fn connect(
        token: &str,
    ) -> Box<Future<Item = (Client, Stream<Item = Event, Error = Error>), Error = Error> + Send>
    {
        let http =
        ...
        Box::new(
            http.request(gateway_req)
                ...
                .and_then(|body| -> Box<Future<Item = _, Error = Error> + Send> {
                    ...
                    println!("{}", result.url);
                    Box::new(
                        try_future_box!(websocket::ClientBuilder::new(&result.url).map_err(|e| {
                            Error::Other(format!("Failed to parse Gateway URI: {:?}", e))
                ...
                .and_then(
                    move |(msg1, socket)| -> Box<Future<Item = _, Error = Error> + Send> {
                        #[derive(Deserialize)]
                        struct Hello {
                        ...
                        if let Some(websocket::message::OwnedMessage::Text(text)) = msg1 {
                            ...
                            Box::new(
                                socket
                                    .send(websocket::message::OwnedMessage::Text(identify))
                                    .map_err(|e| e.into())
